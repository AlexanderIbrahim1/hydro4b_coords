...
class TestSpherical3D:
    ...
    @pytest.mark.parametrize(
        "polar, azimuthal, expect_point",
        [
            (0.5*math.pi, 0.25*math.pi, Cartesian3D(math.sqrt(0.5), math.sqrt(0.5), 0.0)),
            (0.5*math.pi, 0.75*math.pi, Cartesian3D(-math.sqrt(0.5), math.sqrt(0.5), 0.0)),
            (0.5*math.pi, 1.25*math.pi, Cartesian3D(-math.sqrt(0.5), -math.sqrt(0.5), 0.0)),
            (0.5*math.pi, 1.75*math.pi, Cartesian3D(math.sqrt(0.5), -math.sqrt(0.5), 0.0)),
        ]
    )
...
class TestHydrogenMoleculeAtomicPositions:
    @pytest.mark.parametrize(
        ...
        [
            (lebedev.Lebedev3.ORIENT_X, Cartesian3D(1.0, 0.0, 0.0), Cartesian3D(-1.0, 0.0, 0.0)),
            (lebedev.Lebedev3.ORIENT_Y, Cartesian3D(0.0, 1.0, 0.0), Cartesian3D(0.0, -1.0, 0.0)),
            (lebedev.Lebedev3.ORIENT_Z, Cartesian3D(0.0, 0.0, 1.0), Cartesian3D(0.0, 0.0, -1.0)),
        ]
    )
    def test_lebedev3(self, orientation, expect_point0, expect_point1):
        ...
        point0, point1 = hydrogen_molecule_atomic_positions(centre, bondlength, angles.polar, angles.azimuthal)
    ...
    def test_both_points_same_distance_from_centre(self):
        ...
        point0, point1 = hydrogen_molecule_atomic_positions(centre, bondlength, polar, azimuthal)
