...
class TestMRCCInputFileData:
    ...
    def test_set_calculation_type_raises(self):
        ...
        assert "'whatever' is not a valid option for 'set_calculation_type'" in str(
            exc_info.value
        )
    ...
    def test_positive_value_setters_raises(self, methodname, value):
        ...
        assert f"'{methodname}' must accept a positive integer value" in str(
            exc_info.value
        )
...
def get_six_molecules() -> list[molecule.HydrogenMoleculeInfo]:
    ...
    molecules = molecule.get_molecules(
        centres_of_mass, orientations, angle_map, bondlength
    )
...
def test_basis_lines_with_midbond():
    ...
    expect_basis_lines = "basis=mixed\n" "2\n" "aug-cc-pVDZ 1-6\n" "midbond-3s3p2d 7"
